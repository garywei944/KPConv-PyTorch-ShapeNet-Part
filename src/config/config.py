...
class Config:
    ...
    def __init__(self):
        ...
        # Number of layers
        self.num_layers = len([block for block in self.architecture if
                               'pool' in block or 'strided' in block]) + 1
        ...
        for line in lines:
            ...
            if len(line_info) > 2 and line_info[0] != '#':
                ...
                if line_info[2] == 'None':
                    ...
                    self.lr_decays = {
                        int(b.split(':')[0]): float(b.split(':')[1]) for b in
                        line_info[2:]}
                    ...
                    self.augment_symmetries = [bool(int(b)) for b in
                                               line_info[2:]]
